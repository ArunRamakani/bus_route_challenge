...
if [ $# -lt 2 ]
then
   RouteFilePath=data/example
fi

dev_build() {
  # Do what you need to package your app, e.g. mvn package
  mvn clean install -DRouteFilePath=$RouteFilePath
  true
}
...
dev_run() {
  ...
  java -Xmx1024m -DRouteFilePath=$RouteFilePath -jar target/bus-route-challenge-1.0.jar
  sleep 600
}
